...
import { AGConnectCloudDBExceptionCode } from '@hw-agconnect/database';
...
function displayView(type) {
  ...
  if (type === 1) {
    ...
    document.getElementById('formUserKey').style.display = 'none';
    document.getElementById('fromCloudZone').style.display = 'none';
  } else if (type === 2) {
    ...
    document.getElementById('formQuery').style.display = 'none';
    document.getElementById('fromUser').style.display = 'none';
    document.getElementById('formUserKey').style.display = 'block';
    document.getElementById('fromCloudZone').style.display = 'none';
  } else if (type === 5) {
    document.getElementById('formAdd').style.display = 'none';
    document.getElementById('formQuery').style.display = 'none';
    document.getElementById('fromUser').style.display = 'none';
    document.getElementById('formUserKey').style.display = 'none';
    document.getElementById('fromCloudZone').style.display = 'block';
  }
}
...
async function loginByPhone() {
  const state = await agcLoginByPhone(
...
async function openZone() {
  ...
  const result = await openCloudDBZone(zoneName);
  if (result === true) {
    console.log('open zone success');
    await subscribeBookList();
    await queryBooks();
  }
  document.getElementById('AddDiv').style.display = 'none';
}
...
async function setUserkey() {
  await setUserKeys(document.getElementById('userKey').value,
    document.getElementById('userReKey').value);
  document.getElementById('AddDiv').style.display = 'none';
}
